...
interface VaultState {
  ...
  // ── Master password (stored ONLY in memory for decryption) ──────────────
...
function scheduleInactivityLock(): void {
  clearInactivityTimer();
  const inactivityMs =
    useSettingsStore.getState().autoLockTimeInMinutes * 60 * 1000;
...
if (typeof window !== "undefined") {
  document.addEventListener("visibilitychange", () => {
    ...
    if (document.hidden) {
      ...
      const hiddenMs =
        useSettingsStore.getState().hiddenTabTimeoutInMinutes * 60 * 1000;
